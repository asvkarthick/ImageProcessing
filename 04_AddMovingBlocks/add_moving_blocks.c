...
unsigned char *block_buffer_black;
...
int main(int argc, char* argv[])
{
    ...
    block_buffer_black = (unsigned char*) malloc(block_width * block_height * 2);

    fillBuffer(frame_buffer, width, height, 8);
    fillBuffer(block_buffer, block_width, block_height, 9);
    fillBuffer(block_buffer_black, block_width, block_height, 8);
    ...
    for(frame = 0; frame < num_frames; frame++)
    {
        ...
        addBlock(frame_buffer, width, height, block_buffer_black, block_width, block_height, x_next, y_next);
