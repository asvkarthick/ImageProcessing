...
class ImageGenerator {
 ...
 public:
  ...
  bool savePNG(const std::string& filename) {
    ...
    uint32_t ihdrLengthBE = bswap32_manual(ihdrLength);
    file.write(reinterpret_cast<const char*>(&ihdrLengthBE), 4);
    file.write("IHDR", 4);

    uint32_t widthBE = bswap32_manual(width);
    uint32_t heightBE = bswap32_manual(height);
    ...
    uint32_t idatLengthBE = bswap32_manual(idatLength);
